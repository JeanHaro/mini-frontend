...
export function bootstrap(components) {
    for (const Component of components) {
        ...
        const states = Reflect.getMetadata(STATE_KEY, Component) ?? [];
        ...
        function registerEvents() {
            ...
            for (const event of events) {
                element.addEventListener(event.event, (e) => {
                    ...
                    console.log('click en:', target.outerHTML);
                    console.log('matches selector:', event.selector, target.matches(event.selector));
